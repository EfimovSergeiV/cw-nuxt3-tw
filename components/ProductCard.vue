  ...
  <div class="bg-white dark:bg-gray-800 border border-gray-200 hover:border-gray-300 dark:border-gray-700 hover:dark:border-gray-600 transition-all rounded-sm px-2">
    <div class="py-2">
      ...
      <div class="flex items-center justify-between my-2 md:min-h-[50px]">
        ...
        <div class="">
          <p v-if="props.product.only_price === 0" class="text-xs text-right font-medium text-gray-900 dark:text-gray-200">стоимость по запросу</p>
          <div v-else class="flex gap-2 items-start">
            <p class="text-base text-right font-medium text-gray-900 dark:text-gray-200">{{ props.product.only_price.toLocaleString() }}</p>
