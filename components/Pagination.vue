    ...
    <ul class="flex items-center gap-0.5 font-semibold">
      ...
      <li v-for="(page, pk) in pages" :key="pk">
        <nuxt-link :to="{ name: 'prods', query: { ...route.query, page: page }}">
          <div v-if="Number(current) === Number(page)" class="w-8 h-8 flex items-center justify-center text-gray-800 bg-blue-50 border border-gray-300 hover:bg-blue-100 hover:text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white">{{ page }}</div>
